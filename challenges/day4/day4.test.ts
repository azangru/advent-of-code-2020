...
describe('isValid2', () => {
  ...
  const invalidPassports: string = dedent`
  eyr:1972 cid:100
  ...
  const validPassports: string = dedent`
  ...
  test('invalid passports', () => {
    const documents = splitDocuments(invalidPassports);

    documents.forEach((doc) => {
      expect(isValid2(doc)).toBe(false);
    });
  });

  test('valid passports', () => {
    const documents = splitDocuments(validPassports);

    documents.forEach((doc) => {
      expect(isValid2(doc)).toBe(true);
    });
  });
